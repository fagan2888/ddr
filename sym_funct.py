...
def acsf_rad(xyzs, Zs, radial_cutoff, radial_rs, eta):
    ...
    with tf.name_scope("Distances"):
        ...
        dist_tensor = tf.cast(tf.norm(dxyzs+1.e-16, axis=3), dtype=tf.float64)  # (n_samples, n_atoms, n_atoms)
    ...
    with tf.name_scope("fc_term"):
        ...
        zeros = tf.zeros(tf.shape(dist_tensor), dtype=tf.float64)
...
def acsf_ang(xyzs, Zs, angular_cutoff, angular_rs, theta_s, zeta, eta):
    ...
    zeros_1 = tf.zeros(tf.shape(sum_dist_tensor), dtype=tf.float64)
    ...
    with tf.name_scope("Fc_term"):
        ...
        zeros_2 = tf.zeros(tf.shape(dist_tensor), dtype=tf.float64)
    ...
    with tf.name_scope("Theta"):
        ...
        dots_dxyzs = tf.cast(tf.reduce_sum(tf.multiply(tf.expand_dims(dxyzs, axis=3), tf.expand_dims(dxyzs, axis=2)),
                                   axis=4), dtype=tf.float64)  # (n_samples,  n_atoms, n_atoms, n_atoms)
        ...
        # Taking care of the values that due numerical error are just above 1.0 or below -1.0 (additional term for numerical stability of gradients)
        cut_cos_theta = tf.clip_by_value(cos_theta, tf.constant(-1.0 + 1.0e-7, dtype=tf.float64), tf.constant(1.0 - 1.0e-7, dtype=tf.float64))
    ...
    with tf.name_scope("Cos_term"):
        ...
        cos_term = tf.pow(tf.add(tf.ones(tf.shape(cos_theta_term), dtype=tf.float64), cos_theta_term),
    ...
    const = tf.pow(tf.constant(2.0, dtype=tf.float64), (1.0 - zeta))
...
def sum_rad(pre_sum, Zs, elements_list, radial_rs):
    ...
    zeros = tf.zeros(tf.shape(pre_sum), dtype=tf.float64)
    ...
    # clean_final_term = tf.where(mask, final_term, tf.zeros(final_term.shape, dtype=tf.float64))
    clean_final_term = tf.where(mask, final_term, tf.zeros(tf.shape(final_term), dtype=tf.float64))
...
def sum_ang(pre_sumterm, Zs, element_pairs_list, angular_rs, theta_s):
    ...
    zeros_large = tf.zeros(tf.shape(pre_sumterm), dtype=tf.float64, name="zero_large")
    presum_terms = []
...
def generate_parkhill_acsf(xyzs, Zs, elements, element_pairs, radial_cutoff, angular_cutoff,
    ...
    with tf.name_scope("acsf_params"):
        rad_cutoff = tf.constant(radial_cutoff, dtype=tf.float64)
        ang_cutoff = tf.constant(angular_cutoff, dtype=tf.float64)
        rad_rs = tf.constant(radial_rs, dtype=tf.float64)
        ang_rs = tf.constant(angular_rs, dtype=tf.float64)
        theta_s = tf.constant(theta_s, dtype=tf.float64)
        zeta_tf = tf.constant(zeta, dtype=tf.float64)
        eta_tf = tf.constant(eta, dtype=tf.float64)
...
def jac2(y,x, pfor = False):
    return batch_jacobian(y, x, use_pfor = pfor)


def get_numerical_gradients(deviation = 1e-6):
    radial_cutoff = 6.0
    angular_cutoff = 6.0
    radial_rs = np.arange(0.0, 6.5, 5)
    angular_rs = np.arange(0.0, 6.5, 5)
    theta_s = np.arange(0.0, 3.14, 2.5)
    zeta = 1.0
    eta = 1.0
    ...
    xyzs = np.concatenate([xyzs]*10)
    zs = np.concatenate([zs]*10)


    n_atoms = zs.shape[1]
    n_samples = 2 * n_atoms * 3
    n_feat = len(elements) * len(radial_rs) + len(element_pairs) * len(angular_rs) * len(theta_s)
    #print(n_feat)

    tf.reset_default_graph()

    with tf.name_scope("Inputs"):
        zs_tf = tf.placeholder(shape=[None, n_atoms], dtype=tf.int32, name="zs")
        xyz_tf = tf.placeholder(shape=[None, n_atoms, 3], dtype=tf.float64, name="xyz")
    ...
    with tf.name_scope("acsf_params"):
        ...
        rad_rs = tf.constant(radial_rs, dtype=tf.float64)
        ang_rs = tf.constant(angular_rs, dtype=tf.float64)
        theta_s_tf = tf.constant(theta_s, dtype=tf.float64)
        zeta_tf = tf.constant(zeta, dtype=tf.float64)
        eta_tf = tf.constant(eta, dtype=tf.float64)


    full_thing = generate_parkhill_acsf(xyz_tf, zs_tf, elements, element_pairs, radial_cutoff, angular_cutoff, radial_rs, angular_rs, theta_s, zeta, eta)
    ...
    for i in range(xyzs.shape[0]):
        perturbed_xyz = make_pertubations(xyzs[i], deviation)
        repeated_zs = np.zeros([perturbed_xyz.shape[0], zs[i].shape[0]], dtype = int)
        repeated_zs[:] = zs[i]
        acsf_pertubations = sess.run(full_thing, feed_dict={xyz_tf:perturbed_xyz, zs_tf: repeated_zs})
        partial_grad = calculate_numerical_gradient(acsf_pertubations, deviation)
        grad.append(partial_grad)

    return np.asarray(grad)

def get_tf_gradients():
    radial_cutoff = 6.0
    angular_cutoff = 6.0
    radial_rs = np.arange(0.0, 6.5, 0.5)
    angular_rs = np.arange(0.0, 6.5, 0.5)
    theta_s = np.arange(0.0, 3.14, 0.25)
    zeta = 1.0
    eta = 1.0
    ...
    with tf.name_scope("Inputs"):
        ...
        xyz_tf = tf.placeholder(shape=[1, n_atoms, 3], dtype=tf.float64, name="xyz")
    ...
    grad = []
    for i in range(xyzs.shape[0]):
        partial_grad = sess.run(jac_, feed_dict={xyz_tf: xyzs[i:i+1], zs_tf: zs[i:i+1]})
        grad.append(partial_grad)

    return np.asarray(grad)

def get_tf_gradients2(pfor=False):
    zeta = 1.0
    eta = 1.0
    radial_cutoff = 5.0
    angular_cutoff = 5.0
    ...
    jac_ = jac2(full_thing, xyz_tf, pfor = pfor)

    sess = tf.Session()
    sess.run(tf.global_variables_initializer())

    rep, grad = sess.run([full_thing, jac_], feed_dict={xyz_tf: xyzs, zs_tf: zs})

    return rep, grad

if __name__ == "__main__":
    #t = time.time()
    #tf_grad = get_tf_gradients()
    #print(time.time() - t)
    t = time.time()
    rep, tf_grad2 = get_tf_gradients2()
    np.savez("rep.npz", rep)
    np.savez("grad.npz", tf_grad2)
    print(time.time() - t)
    print(rep.shape, tf_grad2.shape)
    quit()
    #t = time.time()
    #tf_grad2 = get_tf_gradients2(pfor=True)
    #print(time.time() - t)
    t = time.time()
    num_grad = get_numerical_gradients(3e-3)
    print(time.time() - t)

    #print("xoxo", grad, np.max(abs(tf_grad2 - num_grad)), time.time() - t)
    #print(np.max(abs(tf_grad - tf_grad2)))
